            ...
            <div onClick={(e) => e.stopPropagation()} className='bg-[#f0f7ff] rounded-xl w-full max-w-md'>
                <form action={formAction} className='flex flex-col'>
                    <div className='p-4 md:p-6'>
                        ...
                        <p className='text-sm text-[#5e758d]'>Create a new profile and assign to an account.</p>
                        <TextField label='Profile Name' placeholder='profile name' className='mt-4' />
                        <TextField label='PIN Code' type='number' placeholder='pin code' className='mt-3' />
                        <TextField label='Payment Url' type='url' placeholder='Payment Url' className='mt-3' />
                        <TextField label='Assign Account' options={accounts} className='mt-3' />
                    </div>
...
function TextField({ label, placeholder, options, value, onChange, className, type = 'text' }) {
    ...
    return (<div className={`${className} flex flex-col w-full`}>
        ...
        <select name={label} required className='border-2 border-[#e4e4e4] bg-white rounded-xl p-3 mt-2'>
            {options.map(option => <option key={option.id} value={option.id}>{option.email}</option>)}
        </select>
    </div>)
}
